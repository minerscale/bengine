...
vec3 day_top_color = vec3( 0.1, 0.6, 1.0 );
vec3 day_bottom_color = vec3( 0.4, 0.8, 1.0 );
vec3 sunset_top_color = vec3( 0.7, 0.75, 1.0 );
vec3 sunset_bottom_color = vec3( 1.0, 0.5, 0.7 );
vec3 night_top_color = vec3( 0.02, 0.0, 0.04 );
vec3 night_bottom_color = vec3( 0.1, 0.0, 0.2 );

vec3 horizon_color = vec3( 0.0, 0.7, 0.8 );
float horizon_blur = 0.05;

vec3 sun_color = vec3( 10.0, 8.0, 1.0 );
vec3 sun_sunset_color = vec3( 10.0, 0.0, 0.0 );
float sun_size = 0.1;
float sun_blur = 3.0;

vec3 moon_color = vec3( 1.0, 0.95, 0.7 );
float moon_size = 0.03;
float moon_blur = 0.1;
...
float clouds_speed = 2.0;
float clouds_direction = 0.2;
float clouds_scale = 1.0;
float clouds_cutoff = 0.3;
float clouds_fuzziness = 0.5;
// More weight is simply a darker color, usefull for rain/storm
float clouds_weight = 0.0;
float clouds_blur = 0.25;

	// Stars should be at black background
//sampler2D stars_texture : filter_linear_mipmap, hint_default_black;
float stars_speed = 1.0;
...
const vec3 LIGHT0_DIRECTION = normalize(vec3(1.0, 1.0, 1.0));
const vec3 LIGHT1_DIRECTION = normalize(vec3(-1.0, 1.0, -1.0));

bool LIGHT0_ENABLED = true;
bool LIGHT1_ENABLED = true;

vec3 sky(vec3 eyedir, float time)
{
	//////////////////// SKY ///////////////////////////////////////////////////////////////////////
...
void main() {
	...
	vec3 dir_camera = normalize(vec3(-scaled.x, -scaled.y, 1.0));
	vec3 dir_world = rotate(dir_camera, camera_rotor);

	//out_color = vec4(sky_color(dir_world), 1.0);

	out_color = vec4(sky(dir_world, view.time), 1.0);
}
